...
// ---------- Motores ----------
const statusOptions = ['ligado', 'desligado', 'falha'];
const nomes = [
  'Elevador 01 - Moega',
  'Elevador 02 - Silo',
  'Transportador 01',
...
export const motoresData = nomes.map((nome, i) => {
  const status = i < 8 ? 'ligado' : i >= 18 ? 'falha' : 'desligado';
  const carga = status === 'ligado' ? Math.floor(Math.random() * 60) + 20 : 0;
  const corrente = status === 'ligado' ? +(Math.random() * 25 + 5).toFixed(1) : 0;
  return {
    id: `M${i + 1}`,
    nome,
    ccm: i < 10 ? 'CCM 1' : 'CCM 2',
    status,
    horimetro: +(Math.random() * 800).toFixed(1),
    corrente,
    cargaEstimada: carga,
    alarme: status === 'falha' ? 'Sobrecarga' : 'OK',
    potenciaNominal: +(Math.random() * 15 + 2).toFixed(1),
  };
});
